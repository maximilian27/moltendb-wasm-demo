...
async function initDB() {
  try {
    ...
    addLog(`✅ MoltenDb ready — Running as ${db.isLeader ? 'Leader' : 'Follower Proxy'}`, 'success');
...
const QUERIES = [
  // ── Seed data ──
  { group: 'Seed Data', num: 1, label: 'Seed memory collection', action: 'set', code:
        `await db.collection('memory').set({
  ...
  { group: 'Seed Data', num: 2, label: 'Seed display collection', action: 'set', code:
        `await db.collection('display').set({
  ...
  { group: 'Seed Data', num: 3, label: 'Seed laptops collection', action: 'set', code:
        `await db.collection('laptops').set({
  ...
  // ── Basic reads ──
  { group: 'Basic Reads', num: 4, label: 'Get single laptop (lp2)', action: 'get', code:
        `await db.collection('laptops').get().keys('lp2').exec()` },

  { group: 'Basic Reads', num: 5, label: 'Get all laptops', action: 'get', code:
        `await db.collection('laptops').get().exec()` },

  { group: 'Basic Reads', num: 6, label: 'Get batch of laptops', action: 'get', code:
        `await db.collection('laptops').get().keys(['lp1','lp3','lp5']).exec()` },

  // ── Field selection ──
  { group: 'Field Selection', num: 7, label: 'Select brand, model, price only', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .exec()` },

  { group: 'Field Selection', num: 8, label: 'Deep nested field (CPU specs)', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'specs.cpu.ghz', 'specs.cpu.cores'])
  .exec()` },

  { group: 'Field Selection', num: 9, label: 'Exclude price and FK fields', action: 'get', code:
        `await db.collection('laptops').get()
  .excludedFields(['price', 'memory_id', 'display_id'])
  .exec()` },

  { group: 'Field Selection', num: 10, label: 'Validation: fields + excludedFields error', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand'])
  ...
  // ── Where ──
  { group: 'WHERE Clause', num: 11, label: 'Exact match — Apple only', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .where({ brand: 'Apple' })
  .exec()` },

  { group: 'WHERE Clause', num: 12, label: 'Numeric range $gt/$lt', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .where({ price: { $gt: 1000, $lt: 2000 } })
  .exec()` },

  { group: 'WHERE Clause', num: 13, label: 'Nested field — 12+ CPU cores', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'specs.cpu'])
  .where({ 'specs.cpu.cores': { $gte: 12 } })
  .exec()` },

  { group: 'WHERE Clause', num: 14, label: 'Multiple conditions — Intel under 2kg', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'specs.cpu.brand', 'specs.weight_kg'])
  .where({ 'specs.cpu.brand': 'Intel', 'specs.weight_kg': { $lt: 2.0 } })
  .exec()` },

  { group: 'WHERE Clause', num: 15, label: '$ne — not Intel', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'specs.cpu.brand'])
  .where({ 'specs.cpu.brand': { $ne: 'Intel' } })
  .exec()` },

  { group: 'WHERE Clause', num: 16, label: '$contains on string — model has "Pro"', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model'])
  .where({ model: { $contains: 'Pro' } })
  .exec()` },

  { group: 'WHERE Clause', num: 17, label: '$contains on array — tagged "gaming"', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'tags', 'price'])
  .where({ tags: { $contains: 'gaming' } })
  .exec()` },

  { group: 'WHERE Clause', num: 18, label: '$in — Apple, Dell or Razer', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .where({ brand: { $in: ['Apple', 'Dell', 'Razer'] } })
  .exec()` },

  { group: 'WHERE Clause', num: 19, label: '$nin — exclude Framework', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .where({ brand: { $nin: ['Framework'] } })
  .exec()` },

  { group: 'WHERE Clause', num: 20, label: 'Combined — in-stock gaming under $2000', action: 'get', code:
        `await db.collection('laptops').get()
  ...
  // ── Logical operators ──
  { group: 'Logical Operators', num: 58, label: '$or — Apple brand OR XPS model', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model'])
  .where({ $or: [{ brand: { $ct: 'Apple' } }, { model: { $ct: 'XPS' } }] })
  .exec()` },

  { group: 'Logical Operators', num: 59, label: '$and — in stock AND price under $2000', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .where({ $and: [{ in_stock: true }, { price: { $lt: 2000 } }] })
  .exec()` },

  { group: 'Logical Operators', num: 60, label: '$or + top-level — in-stock Apple or Dell', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  ...
  // ── Sort ──
  { group: 'Sort', num: 21, label: 'Sort by price asc', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .sort([{ field: 'price', order: 'asc' }])
  .exec()` },

  { group: 'Sort', num: 22, label: 'Sort by price desc', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .sort([{ field: 'price', order: 'desc' }])
  .exec()` },

  { group: 'Sort', num: 23, label: 'Sort by CPU cores desc', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'specs.cpu.cores', 'specs.cpu.ghz'])
  .sort([{ field: 'specs.cpu.cores', order: 'desc' }])
  .exec()` },

  { group: 'Sort', num: 24, label: 'Multi-field sort — brand asc, price asc', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .sort([{ field: 'brand', order: 'asc' }, { field: 'price', order: 'asc' }])
  .exec()` },

  { group: 'Sort', num: 25, label: 'Sort + WHERE — in-stock by weight asc', action: 'get', code:
        `await db.collection('laptops').get()
  ...
  // ── Pagination ──
  { group: 'Pagination', num: 26, label: 'count — 3 cheapest', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .sort([{ field: 'price', order: 'asc' }])
  .count(3)
  ...
  { group: 'Pagination', num: 27, label: 'offset + count — page 2 (skip 2, take 2)', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .sort([{ field: 'price', order: 'asc' }])
  .offset(2)
  ...
  { group: 'Pagination', num: 28, label: 'offset + count + WHERE — in-stock page 2', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .where({ in_stock: true })
  .sort([{ field: 'price', order: 'asc' }])
  ...
  // ── Joins ──
  { group: 'Joins', num: 29, label: 'Join laptops → memory', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .joins([{ alias: 'ram', from: 'memory', on: 'memory_id' }])
  .exec()` },

  { group: 'Joins', num: 30, label: 'Join laptops → display (partial fields)', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model'])
  .joins([{ alias: 'screen', from: 'display', on: 'display_id', fields: ['refresh_hz','panel','size_inch'] }])
  .exec()` },

  { group: 'Joins', num: 31, label: 'Double join — memory + display', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .joins([
  ...
  { group: 'Joins', num: 32, label: 'Join + WHERE on main — in-stock with display', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .where({ in_stock: true })
  ...
  { group: 'Joins', num: 33, label: 'Join + WHERE on joined — OLED or Mini-LED', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  ...
  { group: 'Joins', num: 34, label: 'Join + WHERE — 120Hz+ display', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  ...
  { group: 'Joins', num: 35, label: 'Join + WHERE — upgradeable RAM', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  ...
  { group: 'Joins', num: 36, label: 'Join + sort by display refresh rate desc', action: 'get', code:
        `await db.collection('laptops').get()
  .fields(['brand', 'model', 'price'])
  .joins([{ alias: 'screen', from: 'display', on: 'display_id', fields: ['refresh_hz','panel'] }])
  ...
  // ── Update & Delete ──
  { group: 'Update & Delete', num: 37, label: 'Update lp4 — back in stock, new price', action: 'update', code:
        `await db.collection('laptops').update({ lp4: { in_stock: true, price: 1749 } }).exec()` },

  { group: 'Update & Delete', num: 38, label: 'Update multiple laptops', action: 'update', code:
        `await db.collection('laptops').update({ lp1: { price: 1399 }, lp6: { price: 799 } }).exec()` },

  { group: 'Update & Delete', num: 39, label: 'Delete single laptop (lp6)', action: 'delete', code:
        `await db.collection('laptops').delete().keys('lp6').exec()` },

  { group: 'Update & Delete', num: 40, label: 'Delete multiple laptops', action: 'delete', code:
        `await db.collection('laptops').delete().keys(['lp4','lp5']).exec()` },

  { group: 'Update & Delete', num: 41, label: 'Drop entire laptops collection', action: 'delete', code:
        `await db.collection('laptops').delete().drop().exec()` },

  // ── Extends ──
  { group: 'Extends', num: 50, label: 'Insert lp7 with embedded memory + display', action: 'set', code:
        `await db.collection('laptops').set({
  lp7: {
  ...
  { group: 'Extends', num: 51, label: 'Read lp7 — ram and screen embedded', action: 'get', code:
        `await db.collection('laptops').get().keys('lp7').exec()` },

  { group: 'Extends', num: 52, label: 'Extends with auto-generated key (array)', action: 'set', code:
        `await db.collection('laptops').set([
  { brand: 'HP', model: 'Spectre x360', price: 1599, in_stock: true }
]).extends({ 0: { ram: 'memory.mem2' } }).exec()` },

  // ── Versioning ──
  { group: 'Versioning', num: 45, label: 'Read lp2 — see _v, createdAt, modifiedAt', action: 'get', code:
        `await db.collection('laptops').get().keys('lp2').exec()` },

  { group: 'Versioning', num: 46, label: 'Conflict test — stale _v:1 write skipped', action: 'set', code:
        `await db.collection('laptops').set({
  lp4: { brand: 'Dell', model: 'XPS 15 STALE', price: 999, _v: 1 }
}).exec()` },

  // ── Compact ──
  { group: 'Maintenance', num: 99, label: 'Compact the database log', action: 'compact', code:
        `// Compact sends a raw action via the transport
...
async function runQuery() {
  ...
  if (!code) { addLog('Editor is empty', 'warn'); return; }
